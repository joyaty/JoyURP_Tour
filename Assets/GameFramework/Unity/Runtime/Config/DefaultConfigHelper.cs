...
namespace UnityGameFramework.Runtime
{
    ...
    public class DefaultConfigHelper : ConfigHelperBase
    {
        ...
        public override bool ReadData(IConfigManager configManager, string configAssetName, object configAsset, object userData)
        {
            ...
            LogUtil.Warning("Config asset '{0}' is invalid.", configAssetName);
            return false;
        }
        ...
        public override bool ParseData(IConfigManager configManager, string configString, object userData)
        {
            try
            {
                ...
                while ((configLineString = configString.ReadLine(ref position)) != null)
                {
                    ...
                    {
                        LogUtil.Warning("Can not parse config line string '{0}' which column count is invalid.", configLineString);
                        return false;
                    }
                    ...
                    {
                        LogUtil.Warning("Can not add config with config name '{0}' which may be invalid or duplicate.", configName);
                        return false;
                    }
                }
            ...
            {
                LogUtil.Warning("Can not parse config string with exception '{0}'.", exception);
                return false;
            }
        }
        ...
        public override bool ParseData(IConfigManager configManager, byte[] configBytes, int startIndex, int length, object userData)
        {
            ...
            {
                LogUtil.Warning("Can not parse config bytes with exception '{0}'.", exception);
                return false;
            }
        }
